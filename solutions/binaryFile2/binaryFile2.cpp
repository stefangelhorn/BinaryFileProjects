 ...
 * @brief Beispielcode zum Speichern und Lesen von einer eigenen Datenstruktur mit
 * Strings in eine Binärdatei.
 ...
 */
template< typename T >
void write_obj_binary( const T& obj, std::ostream& bos );

/**
 * Spezialisierte Templatefunktion für T=std::string um diesen binär zu speichern.
 *
 * Dazu wird erst die Anzahl zu speichernder Zeichen, dann die Zeichen selbst
 * geschrieben.
 */
template<>
void write_obj_binary( const std::string& str, std::ostream& bos );

/**
 * Templatefunktion um beliebige Datentypen binär zu laden.
 */
template< typename T >
void read_obj_binary( T& obj, std::istream& bis );
 ...
 */
template<>
void read_obj_binary( std::string& str, std::istream& bis );
...
{
    std::cout << title << ' ' << autor << ' ' << pages << ' ' << release << ' '
              << price << '\n';
}
